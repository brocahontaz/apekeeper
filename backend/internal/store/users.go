...
func (s UserStore) UpsertBattleNetUser(ctx context.Context, bnetID, battletag, accessToken, refreshToken string, expiresAt time.Time) (domain.User, error) {
	var u domain.User
  ...
  SELECT u.id,u.display_name,u.app_role FROM users u JOIN battle_net_identities i ON i.user_id=u.id WHERE i.bnet_id=$1
...
ON CONFLICT(bnet_id) DO UPDATE SET battletag=EXCLUDED.battletag,access_token=EXCLUDED.access_token,refresh_token=EXCLUDED.refresh_token,token_expires_at=EXCLUDED.token_expires_at,updated_at=now()
RETURNING user_id`, bnetID, battletag, accessToken, refreshToken, expiresAt).Scan(&u.ID)
	if err != nil {
		return u, err
	}
	err = s.pool.QueryRow(ctx, `SELECT u.id,u.display_name,u.app_role FROM users u WHERE u.id=$1`, u.ID).Scan(&u.ID, &u.DisplayName, &u.Role)
	return u, err
}
...
func (s UserStore) SessionUser(ctx context.Context, id string) (domain.User, error) {
	var u domain.User
	err := s.pool.QueryRow(ctx, `SELECT u.id,COALESCE(u.display_name,''),i.battletag,u.app_role FROM sessions s JOIN users u ON u.id=s.user_id JOIN battle_net_identities i ON i.user_id=u.id WHERE s.id=$1 AND s.expires_at > now()`, id).Scan(&u.ID, &u.DisplayName, &u.BattleTag, &u.Role)
	return u, err
}
