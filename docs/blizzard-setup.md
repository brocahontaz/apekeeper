...
Register the redirect URI configured by `OAUTH_REDIRECT_URL`. For local development with the supplied environment this is **`http://localhost:8080/api/auth/callback`**. The backend API callback endpoint is `GET /api/auth/callback`; use that exact URL unless you change the backend’s configured redirect URL.
