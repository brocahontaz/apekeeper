...
<section>
  ...
  <table>
    <thead><tr><th>Started</th><th>Trigger</th><th>Status</th><th>Updated</th></tr></thead><tbody
      >{#each runs as run}<tr
          ><td>{relativeTime(run.startedAt)}</td><td>{run.trigger}</td><td
            ><span class="chip">{run.status}</span></td
          ><td>{run.updated}</td></tr
        >{:else}<tr><td colspan="4">No sync runs recorded.</td></tr>{/each}</tbody
    >
  </table>
</section>
<aside class="guidance">
  ...
  <p>
    Set client credentials before your first sync. <a href="/docs/blizzard-setup.md"
      >Read the keeper's setup guide</a
    >.
  </p>
</aside>
