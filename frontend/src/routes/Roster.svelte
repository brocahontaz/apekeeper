<script lang="ts">
  ...
  let error = $state("");
  let filtered = $derived(filterRoster(rows, { search, class: klass, stale }));
  let classes = $derived([...new Set(rows.map((x) => x.className))]);
  onMount(async () => {
    try {
      rows = await client.roster(location.search);
    } catch (e) {
...
<div class="toolbar">
  <input
    ...
    bind:value={search}
    oninput={update}
  /><select aria-label="Class" bind:value={klass} onchange={update}
    ><option value="">All classes</option>{#each classes as c}<option>{c}</option>{/each}</select
  ><label><input type="checkbox" bind:checked={stale} onchange={update} /> Stale only</label><button
    onclick={() => {
      search = "";
      klass = "";
      stale = false;
...
{#if error}<p class="error">{error}</p>{:else if !rows.length}<p class="skeleton">
    Loading roster…
  </p>{:else}<p class="count">{filtered.length} keepers found</p>
  <RosterTable rows={filtered} />{/if}
